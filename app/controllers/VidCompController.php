...
class VidCompController extends \BaseController {
	public function __construct(Challenge $challenge)
	{
		parent::__construct();
		Breadcrumbs::addCrumb('Manage Video Competitions', route('vidcomp.index'));
	}
	 ...
	 * @return Response
	 */
	public function index($filter_year = 0)
	{
	    if($filter_year) {
		    $compyears = CompYear::where('year', $filter_year)
		                         ->orderBy('year', 'desc')
		                         ->with('vid_competitions', 'vid_competitions.divisions')->get();
		} else {
		    $compyears = CompYear::orderBy('year', 'desc')
		                         ->with('vid_competitions', 'vid_competitions.divisions')->get();
		}

		View::share('title', 'Manage Video Competitions');
		return View::make('vidcomp.index', compact('compyears'));
	}
