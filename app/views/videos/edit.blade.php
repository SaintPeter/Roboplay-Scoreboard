...
{{ Form::model($video, array('method' => 'PATCH', 'route' => array('videos.update', $video->id), 'role'=>"form", 'class' => 'col-md-6')) }}
	<div class="form-group">
	    ...
	    {{ Form::text('name', Input::old('name', $video->name), [ 'class'=>'form-control col-md-4' ]) }}
	</div>

	<div class="form-group">
	    {{ Form::label('yt_code', 'YouTube URL or Code:') }}
	    {{ Form::text('yt_code', Input::old('yt_code', $video->yt_code), [ 'class'=>'form-control col-md-4' ]) }}
	</div>

	<div class="form-group">
		{{ Form::label('vid_division_id', 'Division:') }}
		{{ Form::select('vid_division_id', $vid_divisions, Input::old('vid_division_id', $video->vid_division_id), [ 'class'=>'form-control col-md-4' ]) }}
	</div>
	...
	<div class="form-group">
	    ...
	    {{ Form::textarea('students', Input::old('students', $video->students)) }}
	    <p>One Student Per Line</p>
	</div>

	<div class="form-group">
	    {{ Form::label('has_custom', 'Has a Custom Part:') }}
	    {{ Form::select('has_custom', [ 0 => 'No', 1 => 'Yes' ], Input::old('has_custom', $video->has_custom)) }}
	</div>
